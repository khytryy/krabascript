...
#define REQUIRED_ARGS \
    REQUIRED_STRING_ARG(input_file, "input", "Input file")

#define OPTIONAL_ARGS \
    OPTIONAL_STRING_ARG(output_file, "a.out", "-o", "Output file", "Output file name") \
    OPTIONAL_UINT_ARG(optimization_lvl, 1, "-O", "Optimization", "Optimization level") \
...
int main(int argc, char* argv[]) {
    ...
    if (Args.comtime == true) {
        ...
        BWHT, BGRN, COLOR_RESET, BMAG, TokenizerTime, COLOR_RESET);
    }
